"""
Train V5 Transformer Models with Advanced Features for All 8 Banking Stocks

Enhanced version that uses:
  - Technical Indicators (35 features)
  - FinBERT Sentiment (4 features)
  - Advanced Market Signals (12 features)
  = 51 Total Features

Advanced signals include:
  - Technical bullish/bearish patterns from news
  - Analyst ratings (buy/hold/sell)
  - Macroeconomic policy signals
  - Risk indicators (NPA, regulatory, competition)
  - Leadership and governance signals
  - Earnings beat/miss indicators
"""
...
print("=" * 80)
print("TRAINING V5 TRANSFORMER WITH ADVANCED SIGNALS - ALL 8 STOCKS".center(80))
print("=" * 80)
...
def train_transformer_model(ticker, stock_name):
    """Train V5 Transformer model with advanced features for a single stock"""
    ...
    try:
        ...
        print(f"\n1. Loading advanced features data...")
        pkl_file = f'data/advanced_model_ready/{ticker}_advanced.pkl'

        if not os.path.exists(pkl_file):
            print(f"   ✗ Advanced data not found: {pkl_file}")
            return None
        ...
        print(f"   ✓ Features: {n_features} (Technical + FinBERT + Advanced Signals)")
        print(f"   ✓ Timesteps: {n_timesteps}")
        print(f"   ✓ Train: {len(y_train)}, Val: {len(y_val)}, Test: {len(y_test)}")

        # Identify signal features
        technical_sig_features = [f for f in feature_names if 'technical_signal' in f.lower() or
                                 'bullish' in f.lower() or 'bearish' in f.lower()]
        analyst_features = [f for f in feature_names if 'analyst' in f.lower() or 'rating' in f.lower()]
        macro_features = [f for f in feature_names if 'macro' in f.lower()]
        risk_features = [f for f in feature_names if 'risk' in f.lower()]

        print(f"   ✓ Advanced signal features detected:")
        if technical_sig_features:
            print(f"      - Technical signals: {len(technical_sig_features)}")
        if analyst_features:
            print(f"      - Analyst ratings: {len(analyst_features)}")
        if macro_features:
            print(f"      - Macro signals: {len(macro_features)}")
        if risk_features:
            print(f"      - Risk signals: {len(risk_features)}")
        ...
        os.makedirs(f'models/saved_v5_advanced/{ticker}', exist_ok=True)
        ...
        results = {
            'stock': stock_name,
            'ticker': ticker,
            'approach': 'v5_transformer_advanced',
        ...
        with open(f'models/saved_v5_advanced/{ticker}/results.json', 'w') as f:
            json.dump(results, f, indent=2)

        # Save scaler for inference
        import shutil
        scaler_src = f'data/advanced_model_ready/{ticker}_advanced.pkl'
        scaler_dst = f'models/saved_v5_advanced/{ticker}/scaler.pkl'
        ...
        print(f"   ✓ Results saved to models/saved_v5_advanced/{ticker}/")
...
print("\n\n" + "=" * 80)
print("V5 TRANSFORMER WITH ADVANCED SIGNALS - ALL STOCKS SUMMARY".center(80))
print("=" * 80)
...
if all_results:
    ...
    summary = {
        ...
        'approach': 'v5_transformer_advanced_all_stocks',
        'architecture': 'Transformer with 51 features (Technical + FinBERT + Advanced Signals)',
    ...
    with open('models/saved_v5_advanced/all_stocks_summary.json', 'w') as f:
        json.dump(summary, f, indent=2)

    print(f"\n✓ Summary saved to: models/saved_v5_advanced/all_stocks_summary.json")

    print("\n" + "=" * 80)
    print("✓ V5 TRANSFORMER WITH ADVANCED SIGNALS TRAINING COMPLETE!".center(80))
    print("=" * 80)
    print(f"\nKey Results:")
    print(f"  - Average Directional Accuracy: {avg_dir_acc:.2f}%")
    print(f"  - Average Confidence: {avg_confidence:.2f}%")
    print(f"  - Models saved to: models/saved_v5_advanced/")
    print(f"\nFeature Enhancement:")
    print(f"  - Technical Indicators: 35")
    print(f"  - FinBERT Sentiment: 4")
    print(f"  - Advanced Signals: 12 (technical patterns, analyst ratings, macro, risk)")
    print(f"  - Total Features: 51")
    print(f"\nNext steps:")
    print(f"  1. Compare with FinBERT-only models")
    print(f"  2. If performance improves, upload to HuggingFace")
    print(f"  3. Update GitHub Actions workflow to use advanced models")
else:
    print("\n✗ No stocks trained successfully")
    print("Check that advanced feature preparation completed successfully")
